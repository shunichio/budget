...
import { Check, Loader2, MessageCircle, Pencil, Plus, QrCode, Send, Sparkles, Trash2, HardDriveUpload } from 'lucide-react'
import { api } from '../api'
...
export function SettingsPage() {
  ...
  const lastRunText = (() => {
    ...
    return ok ? `${t('settings_backupLastRun')}: ${time}` : t('settings_backupLastFail', { msg: boot.settings.backupLastResult ?? '' }) + ` (${time})`
  })()
    ...
    <div className="mx-auto w-full max-w-4xl space-y-5 px-4 py-6 md:px-6 md:py-8">
      ...
      <Card title={t('settings_aiSection')} desc={t('settings_aiDesc')}>
        ...
        <Field label={t('settings_apiKey')}>
          <input className={inputCls} type="password" autoComplete="off" value={aiKey} onChange={e => setAiKey(e.target.value)} placeholder="sk-…" />
        </Field>
        <div className="mb-4">
          ...
          <textarea className={`${inputCls} min-h-[96px] resize-y`} value={aiExtraPrompt} maxLength={20000} onChange={e => setAiExtraPrompt(e.target.value)} placeholder={t('settings_extraPromptPlaceholder')} />
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`ml-auto h-2 w-2 rounded-full ${configured ? 'bg-emerald-400' : 'bg-slate-300'}`} title={configured ? t('settings_testOk') : t('chat_notConfigured')} />
        </div>
      </Card>
      ...
      <Card title={t('settings_backupSection')} desc={t('settings_backupDesc')}>
        <label className="mb-3 flex items-center gap-2 text-xs font-medium text-slate-500">
          <input aria-label={t('settings_backupEnable')} type="checkbox" checked={backupEnabled} onChange={e => setBackupEnabled(e.target.checked)} className="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500" />
          {t('settings_backupEnable')}
        </label>
        <Field label={t('settings_backupTime')}>
          <input aria-label={t('settings_backupTime')} type="time" className={`${inputCls} w-32`} value={backupTime} onChange={e => setBackupTime(e.target.value)} />
        </Field>
        ...
        <div className="grid grid-cols-1 gap-x-6 sm:grid-cols-2">
          ...
          <Field label={t('settings_backupPrefix')}>
            <input className={inputCls} value={r2Prefix} onChange={e => setR2Prefix(e.target.value)} placeholder="al1abb-budget-backup" />
          </Field>
          ...
          <Field label={t('settings_backupSecretKey')}>
            <input className={inputCls} type="password" autoComplete="off" value={r2SecretKey} onChange={e => setR2SecretKey(e.target.value)} placeholder={boot?.settings.backupR2HasSecret ? t('settings_backupSecretPlaceholder') : ''} />
          </Field>
        </div>
      ...
      <Card title={t('settings_imSection')} desc={t('settings_imDesc')}>
        <div className="space-y-2">
          {(channels ?? []).map(ch => (
            <ChannelRow key={ch.id} channel={ch} onChanged={loadChannels} onEdit={() => setEditing(ch)} onLogin={() => setLoginFor(ch.id)} />
          ))}
          {channels && channels.length === 0 && <p className="py-6 text-center text-xs leading-relaxed text-slate-300">{t('settings_noChannels')}</p>}
        </div>
        <div className="mt-4">
...
function ChannelRow({ channel, onChanged, onEdit, onLogin }: { channel: ImChannel; onChanged: () => Promise<void>; onEdit: () => void; onLogin: () => void }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-slate-100 bg-white px-3.5 py-2.5 transition-colors hover:border-slate-200">
      <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${channel.type === 'telegram' ? 'bg-sky-50 text-sky-500' : 'bg-emerald-50 text-emerald-600'}`}>
        <Icon size={15} />
      </span>
      <div className="min-w-0 flex-1">
        <div className="truncate text-[13px] font-medium text-slate-700">{channel.name}</div>
        <div className="truncate text-[11px] text-slate-300">{channel.type === 'wechat' ? (wxUserId ? `${typeLabel} · ${wxUserId}` : t('settings_notBound')) : typeLabel}</div>
      </div>
      {channel.type === 'wechat' && (
      ...
      <button aria-label={channel.id} role="switch" aria-checked={channel.enabled} disabled={busy} onClick={toggle} className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full transition-colors disabled:opacity-50 ${channel.enabled ? 'bg-emerald-500' : 'bg-slate-200'}`}>
        <span className={`ml-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${channel.enabled ? 'translate-x-4' : ''}`} />
      </button>
      <div className="flex shrink-0 gap-0.5">
        <button aria-label={`edit-${channel.id}`} className="rounded-md p-1.5 text-slate-300 hover:bg-slate-100 hover:text-slate-600" onClick={onEdit}>
          <Pencil size={13} />
        </button>
        <button aria-label={`del-${channel.id}`} className="rounded-md p-1.5 text-slate-300 hover:bg-rose-50 hover:text-rose-500" onClick={remove}>
          <Trash2 size={13} />
        </button>
      </div>
    </div>
...
function ChannelModal({ channel, onClose, onSaved }: { channel: ImChannel | null; onClose: () => void; onSaved: () => void }) {
  const { t, toast } = useApp()
  ...
  return (
    <Modal title={channel ? t('settings_editChannel') : t('settings_addChannel')} onClose={onClose}>
      ...
      {isWechat ? (
        <>
          {channel?.type === 'wechat' && channel.config.userId && <p className="mb-3 text-xs leading-relaxed text-slate-500">{t('settings_boundAccount', { id: channel.config.userId })}</p>}
          <p className="rounded-lg bg-sky-50 px-3 py-2 text-[11px] leading-relaxed text-sky-700">{t('settings_scanHint')}</p>
        </>
      ) : (
        <>
...
function WechatLoginModal({ channel, onClose, onConfirmed }: { channel: ImChannel | null; onClose: () => void; onConfirmed: () => void }) {
  const { t, toast } = useApp()
  ...
  const statusText = state?.status === 'need_verifycode' ? t('settings_verifyPrompt') : state?.message || state?.error || ''
      ...
      <div className="flex flex-col items-center">
        ...
        {state?.status === 'need_verifycode' && (
          <div className="mt-1 flex w-full items-center gap-2">
            <input aria-label={t('settings_verifyPrompt')} className={inputCls} value={code} inputMode="numeric" onChange={e => setCode(e.target.value.replace(/\D/g, '').slice(0, 8))} onKeyDown={e => e.key === 'Enter' && submitCode()} placeholder="1234" />
