...
export function buildSchemaDoc() {
  ...
  for (const t of tables) {
    ...
    const cols = db.pragma(`table_info(${ident})`).map(c => {
...
export function createSession(title, meta = {}) {
  const id = uid()
  const now = nowIso()
  db.prepare('INSERT INTO chat_sessions(id,title,channel,external_id,created_at,updated_at) VALUES(?,?,?,?,?,?)').run(id, title, meta.channel || 'web', meta.externalId ?? null, now, now)
...
export function getSessionMessages(sessionId) {
  return db.prepare('SELECT * FROM chat_messages WHERE session_id=? ORDER BY created_at, rowid').all(sessionId).map(transformMsg)
}
...
function transformMsg(m) {
  return {
    ...
    pending: m.resolved === 0 && m.pending_sql ? { sql: m.pending_sql, purpose: m.pending_purpose, index: m.pending_index ?? 0 } : null,
...
export async function confirmPending(sessionId, approve) {
  const row = db.prepare('SELECT * FROM chat_messages WHERE session_id=? AND resolved=0 AND pending_sql IS NOT NULL ORDER BY rowid DESC LIMIT 1').get(sessionId)
